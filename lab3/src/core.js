...
function isInteger(n) {
    // Проверка целого числа через побитовую операцию: преобразование к 32-битному int
    return (n | 0) === n;
}
 ...
 */
function isBinary(n) {
    // Степень двойки: n > 0 и у числа ровно один установленный бит
    return n > 0 && (n & (n - 1)) === 0;
}
...
function deepEqual(firstObject, secondObject) {
    ...
    // Быстрое сравнение по ссылке/значению
    if (firstObject === secondObject) return true;

    // Если типы отличаются или один из них null/undefined
    if (
        firstObject === null || secondObject === null ||
        typeof firstObject !== typeof secondObject
    ) {
        return false;
    }

    // Для не-объектов (включая функции), на этом этапе равенства по ссылке не было
    if (typeof firstObject !== 'object') {
        return false;
    }

    // Сравнение объектов/массивов по набору ключей и значений
    const keysA = Object.keys(firstObject);
    const keysB = Object.keys(secondObject);
    if (keysA.length !== keysB.length) return false;

    // Проверяем, что набор ключей одинаковый (порядок не важен)
